 ...
 */
package com.sxgokit.rdf.web.controller.generator;
...
import com.sxgokit.rdf.util.generator.constants.Constant;
import com.sxgokit.rdf.mapper.generator.GeneratorDao;
import com.sxgokit.rdf.util.generator.SafeProperties;
import com.sxgokit.rdf.util.generator.ZipUtil;
import com.sxgokit.rdf.util.generator.CustomGeneratorFacade;
...
public class DataBaseController extends BaseController {
    ...
    public String findAdminList(HttpServletRequest request) {
        try {
            ...
            List<Map<String, Object>> list = generatorDao.findPageList(page,tableName);
            if (list == null) {
                logger.error("生成代码模块-->查询数据库表失败，date=" + new Date());
            }
            request.setAttribute("page", page);
            request.setAttribute("tableName", tableName);
            request.setAttribute("data_list", list);
        } catch (Exception e) {
            logger.error("生成代码模块-->查询数据库表异常：" , e);
            return page_404;
        }
    ...
    @ResponseBody
    public String genTable(@PathVariable("tableName") String tableName) throws IOException {
        // 配置表名
...
//        }
        if (genCode(tableNames, outRoot)) {
            ...
            if (array.length - 1 > 0) {
                return JSONUtil.toJsonStr(new ResponseBean(HttpStatus.OK.value(), "生成代码成功", osName));
            }
            return JSONUtil.toJsonStr(new ResponseBean(HttpStatus.OK.value(), "生成代码成功", null));
        }
        return JSONUtil.toJsonStr(new ResponseBean(HttpStatus.OK.value(), "生成失败，请检查数据库是否连接正常及表名是否正确以及权限是否缺失", null));
    }
    ...
    public void genTableToZip(@PathVariable("tableName") String tableName,
        ...
        if (genCode(tableNames, tempDir.getPath())) {
            ...
            logger.error("生成代码模块-->生成失败，请检查数据库是否连接正常及表名是否正确以及权限是否缺失，date=" + new Date());
        }
    }
    ...
    public String open() {
        try {
            ...
            logger.error("生成代码模块-->操作失败，date=" + new Date());
        }
        return JSONUtil.toJsonStr(new ResponseBean(HttpStatus.OK.value(), "操作成功", null));
    }
    ...
    public String getConfig(HttpServletRequest request) {
        ...
        try {
            ...
            logger.error("生成代码模块-->获取代码生成配置信息异常：" , e);
            return page_404;
        }
    ...
    /**
     * 刷新/更改配置文件generator.properties
     * @author dolyw.com
     * @date 2019-04-05 18:00:26
     */
    @RequestMapping(value="/optConfig", method=RequestMethod.POST)
    @ResponseBody
    public String optConfig(@RequestParam Map<String, String> config) {
        try {
            final Enumeration urls = DataBaseController.class.getClassLoader().getResources("config/generator.properties");
            while (urls.hasMoreElements()) {
                final URL url = (URL) urls.nextElement();
                    ...
                    if (Boolean.parseBoolean(config.get("isRead"))) {
                        ...
                        while (entries.hasNext()) {
                            ...
                            if (Constant.TEMPLATE.equals(entry.getKey())) {
                                ...
                                File file = new File(projectPath + entry.getValue() + "/macro.include");
                                if (!file.exists()) {
                                    // 模板代码位置不存在
                                    return JSONUtil.toJsonStr(new ResponseBean(HttpStatus.BAD_REQUEST.value(), "当前填写的模板代码位置不存在", null));
                                }
                            }
            ...
            logger.error("生成代码模块-->刷新配置文件失败，date=" + new Date());
        }
        return JSONUtil.toJsonStr(new ResponseBean(HttpStatus.OK.value(), "操作成功", config));
    }
    ...
    public boolean genCode(String[] tableNames, String outRoot) throws IOException {
        ...
        generator.addTemplateRootDir(projectPath + template);
        // 开始执行
        try {
    ...
    @ResponseBody
    public String detail(@PathVariable("tableName") String tableName) {
        // 查询表信息
        ...
        return JSONUtil.toJsonStr(new ResponseBean(HttpStatus.OK.value(), "查询成功", columns));
    }
}